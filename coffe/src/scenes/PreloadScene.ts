...
export default class PreloadScene extends Container {
    ...
    private _loadAssets: LoadAssets = new LoadAssets();
    ...
    private async _startLoadingAssets(app: Application) {
        await this._loadAssets.loadManifest(this._assetsInlineHelper.manifest);
       // await Assets.load(groboldFont);

        await this._loadAssets.loadBundle('fonts').then(() => {
            this._loadAssets.loadBundle('load-screen').then(() => {
    ...
    private _loadGameAssets(mask: Graphics, barAssetsContainer: Container): void {
        this._loadAssets.loadBundle('game-screen', (progress) => {
