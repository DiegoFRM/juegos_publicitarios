...
export default class TutorialHand extends Sprite {
    ...
    public async showTutorialObjects(objects: (Sprite | Container)[]): Promise<void> {
        if (this._running) return;
        this._running = true;
        this.position.set(objects[0].x, objects[0].y);
        await gsap.to(this, { alpha: 1, duration: 0.5 });

        while (this._running) {
            for (const object of objects) {
                if (!this._running) break;
                await this._moveToObject(object);
            }
        }
    ...
    private _moveToObject(object: Sprite | Container): Promise<void> {
